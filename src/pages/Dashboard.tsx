...
import {
  ...
  CYCLE_WEEKLY, PIPELINE_STAGES, RECENT_ACTIVITY, FINANCIAL_SNAPSHOT, fmtEGP,
} from "@/data/mock";
import {
...
function KPI({ label, value, sub, trend, href, icon: Icon, highlight }: KPIProps) {
  return (
    <Link
      to={href}
      className="ejb-card ejb-card-hover p-4 group flex flex-col gap-3 min-w-0"
    >
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 mb-5">
        <KPI label="Active members" value="412" sub={`of ${TOTAL_MEMBERS}`} trend={{ dir: "up", text: "+8 vs last cycle" }} href="/members" icon={Users} />
        ...
        <KPI label="Outstanding dues" value={fmtEGP(outstanding, { compact: true })} sub={`${UNPAID_COUNT} unpaid`} trend={{ dir: "neutral", text: `Closes ${CYCLE_CLOSE}` }} href="/payments" icon={AlertCircle} highlight />
      ...
      {/* Row 2: Attention + Cycle progress */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-5">
        ...
        <div className="ejb-card p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] text-muted-foreground">Last cycle finished at 91%</span>
          </div>
          <div className="num">
          ...
          <p className="text-[11px] text-muted-foreground">Cumulative collections by week through the cycle.</p>
        </div>
      </div>
      ...
      <div className="ejb-card p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[11px] text-muted-foreground">YTD · Finance role only</span>
        </div>
